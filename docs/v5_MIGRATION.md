# Jungle MCP v5 Migration Guide

This guide will help you migrate from the legacy UnityMcpBridge installation to the new JungleMCP package structure in version 5.

## Overview

Version 5 introduces a new package structure. The package is now installed from the `JungleMCP` folder instead of the legacy `UnityMcpBridge` folder.
...
3. Enter the following URL: `https://github.com/CoplayDev/unity-mcp.git?path=/JungleMCP`
4. Click **Add** to install the package

![Installing from the new JungleMCP path](images/v5_02_install.png)

### Step 3: Rebuild MCP Server

After installing the new package, you need to rebuild the MCP server:

1. In Unity, go to **Window > Jungle MCP > Open MCP Window**
...
- Check that the package appears in the Package Manager as **Jungle MCP**
- Confirm the package location shows the new `JungleMCP` path
