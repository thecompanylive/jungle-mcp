...
async def test_apply_text_edits_long_file(monkeypatch):
    ...
    ctx = DummyContext()
    resp = await apply_edits(ctx, "junglemcp://path/Assets/Scripts/LongFile.cs", [edit])
...
async def test_sequential_edits_use_precondition(monkeypatch):
    ...
    resp1 = await apply_edits(DummyContext(), "junglemcp://path/Assets/Scripts/File.cs", [edit1])
    edit2 = {"startLine": 2, "startCol": 0, "endLine": 2,
             "endCol": 0, "newText": "//second\n"}
    resp2 = await apply_edits(
        DummyContext(),
        "junglemcp://path/Assets/Scripts/File.cs",
        [edit2],
