...
namespace Squido.JungleMCP.Editor.Tools
{
    ...
    /// </remarks>
    [JungleMcpTool("manage_script", AutoRegister = false)]
    public static class ManageScript
    {
                ...
                case "get_sha":
                    {
                        try
                        {
                            ...
                            var data = new
                            {
                                uri = $"junglemcp://path/{relativePath}",
                                path = relativePath,
                                sha256 = sha,
                                lengthBytes,
        ...
        )
        {
            ...
            try
            {
                ...
                var uri = $"junglemcp://path/{relativePath}";
    ...
    static class ManageScriptRefreshHelpers
    {
        public static string SanitizeAssetsPath(string p)
        {
            ...
            p = p.Replace('\\', '/').Trim();
            if (p.StartsWith("junglemcp://path/", StringComparison.OrdinalIgnoreCase))
                p = p.Substring("junglemcp://path/".Length);
